...
def test_byte_equivalent_to_pre_refactor_bert_output():
    """The shared helper must produce IDENTICAL output to the BERT path's
    pre-refactor inline truncation logic. Baseline captured via
    /tmp/bert_dim_baseline.py (see plan Phase 1 Task 1.4 Step 2)."""
    vec = np.array([0.5, 0.5, 0.5, 0.5], dtype=np.float32)
    out = apply_dimensions(vec, 2)
    # Baseline output from pre-refactor inline code:

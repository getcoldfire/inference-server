...
def _make_repo(root: Path, repo_id: str, config: dict[str, Any] | None,
               has_safetensors: bool = True) -> Path:
...
def fake_cache(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    _make_repo(tmp_path, "mlx-community/Llama-3.2-1B-Instruct-4bit",
               {"model_type": "llama", "quantization": {"group_size": 64}})
    _make_repo(tmp_path, "Qwen/Qwen2.5-7B-Instruct",
               {"model_type": "qwen2"})  # MLX-compat model_type, no namespace
    _make_repo(tmp_path, "sentence-transformers/all-MiniLM-L6-v2",
               {"model_type": "bert"})  # non-MLX

    repos = [
        {"repo_id": "mlx-community/Llama-3.2-1B-Instruct-4bit",
         "size": 712_000_000,
         "mtime": datetime(2026, 6, 5, 14, 0, tzinfo=timezone.utc).timestamp(),
         "path": tmp_path / "models--mlx-community--Llama-3.2-1B-Instruct-4bit"},
        {"repo_id": "Qwen/Qwen2.5-7B-Instruct",
         "size": 4_400_000_000,
         "mtime": datetime(2026, 6, 8, 9, 0, tzinfo=timezone.utc).timestamp(),
         "path": tmp_path / "models--Qwen--Qwen2.5-7B-Instruct"},
        {"repo_id": "sentence-transformers/all-MiniLM-L6-v2",
         "size": 90_000_000,
         "mtime": datetime(2026, 5, 1, 12, 0, tzinfo=timezone.utc).timestamp(),
...
@pytest.mark.parametrize("config,namespace,expected", [
...
def test_is_mlx_shaped(config, namespace, expected, tmp_path,

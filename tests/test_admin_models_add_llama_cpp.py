...
def test_valid_model_types_includes_llama_cpp():
    ...
    assert "llama-cpp" in VALID_MODEL_TYPES, f"'llama-cpp' missing from VALID_MODEL_TYPES: {VALID_MODEL_TYPES}"
...
async def test_admin_post_llama_cpp_passes_hf_file_to_model_cfg_dict():
    ...
    assert cfg.get("hf_file") == "nomic-embed-text-v1.5.f16.gguf", f"hf_file missing or wrong in model_cfg_dict: {cfg}"
    assert cfg.get("n_gpu_layers") == -1, f"n_gpu_layers missing or wrong in model_cfg_dict: {cfg}"
    assert cfg.get("n_ctx") == 8192, f"n_ctx missing or wrong in model_cfg_dict: {cfg}"
    assert cfg.get("n_batch") == 512, f"n_batch missing or wrong in model_cfg_dict: {cfg}"
    assert cfg.get("n_threads") == 4, f"n_threads missing or wrong in model_cfg_dict: {cfg}"

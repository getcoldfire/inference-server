...
def _remap_hf_to_internal(weights: dict[str, Any]) -> dict[str, Any]:
    ...
    - Strip a leading `model.` prefix (some sentence-transformers repos add it).
    - `.layers.` (plural) -> `.layer.` (HF BERT singular).
    - `.attn.Wqkv.` (nomic-bert combined QKV projection) -> split into
      separate `.attention.self.{query,key,value}.` matrices of shape (H, H)
      each. Both `weight` and `bias` are split if present.
    - `.attn.out_proj.` -> `.attention.output.dense.`
    - `.mlp.fc1.` -> `.intermediate.dense.`
    - `.mlp.fc2.` -> `.output.dense.`
    - `.mlp.gate_proj.` / `.mlp.up_proj.` / `.mlp.down_proj.` -> nomic SwiGLU
      keys `.mlp.gate.` / `.mlp.up.` / `.mlp.down.`.

    The exact remap rules will need empirical refinement once we run the
    real `nomic-embed-text-v1.5` weights through; this version covers the
    documented HF conventions and is enough to keep the architecture aligned.
    """
    out: dict[str, Any] = {}
...
def load_embedding_model(
    ...
    # ndarrays so the remap can do shape-aware splits like Wqkv).
    remapped = _remap_hf_to_internal(raw_weights)
